...
class InfoSQLController(http.Controller):
    ...
    def info_sql(self, **kwargs):
        ...
        for payload in payloads:
            ...
            table_ids = self._process_schema_info(db_rec, payload.get('schema_info', []), today)
            rows_updated = self._process_row_counts(db_rec, payload.get('table_row_counts', []), today)
    ...
    def _get_or_create_database(self, db_name, sql_server, host):
        ...
        if db_rec:
            db_rec.write({'name': db_name, 'sql_seversql': sql_server})
        else:
    ...
    def _process_schema_info(self, db_rec, schema_info, today):
        """Xử lý schema_info: tạo/cập nhật table + column"""
        tables_cache, table_ids = {}, []

        for s in schema_info:
            table_name, col_name, data_type = s.get('TableName'), s.get('ColumnName'), s.get('DataType')
            if not table_name:
                continue

            # --- xử lý bảng ---
            table_rec = self._get_or_create_table(db_rec, table_name, today, tables_cache)

            # --- xử lý column ---
            if col_name:
                self._get_or_create_column(db_rec, table_name, col_name, data_type)

            table_ids.append(table_rec.id)

        return table_ids

    def _get_or_create_table(self, db_rec, table_name, today, tables_cache):
        """Tìm hoặc tạo mới 1 table.sql, có cache"""
    ...
    def _get_or_create_column(self, db_rec, table_name, col_name, data_type):
        """Tìm hoặc tạo mới 1 column.sql"""
    ...
    def _process_row_counts(self, db_rec, row_counts, today):
        """Cập nhật sum_record trong table.sql"""
        Table = request.env['table.sql'].sudo()
        table_cache, updated = {}, 0

        for r in row_counts:
            table_name, total_rows = r.get('TableName'), r.get('TotalRows')
            if not table_name:
                continue
            ...
            # update sum_record
            table_cache[cache_key].write({'sum_record': total_rows or 0})
            updated += 1
